...
let rec to_burst_string ?(d=0) (e:t) : string =
  space_string d
  ^
  match e with
  | EVar id -> id
  | ETuple es ->
      "(" ^ (es |> List.map ~f:to_burst_string |> String.concat ~sep:", ") ^ ")"
  (* | EOp (op, es) ->
      "(" ^ Op.str op ^ " "
      ^ (es |> List.map ~f:to_burst_string |> String.concat ~sep:" ")
      ^ ")" *)
  | EOp (Cons, [ hd; tl ]) -> "Cons (" ^ to_burst_string hd ^ ", " ^ to_burst_string tl ^ ")"
  | EOp (Node, [ v; t1; t2 ]) -> "Node (" ^ to_burst_string v ^ ", " ^ to_burst_string t1 ^ ", " ^ to_burst_string t2 ^ ")"
  | EOp (Succ, [ e ]) -> "S (" ^ to_burst_string e ^ ")"
  | EOp (Zero, []) -> "0"
  | EOp (op, es) ->
      "(" ^ Op.str op ^ " "
      ^ (es |> List.map ~f:to_burst_string |> String.concat ~sep:" ")
      ^ ")"
  | EApp (f, arg) -> "(" ^ to_burst_string f ^ " " ^ to_burst_string arg ^ ")"
  | EMatch (scrut, branches) ->
      "match (" ^ to_burst_string scrut ^ ") with\n"
      ^ (branches
        |> List.map ~f:(fun (p, e) ->
               space_string d ^ "| " ^ Pat.to_burst_string p ^ " -> \n"
               ^ to_burst_string ~d:(d + 1) e
               ^ "\n")
        |> String.concat)
  | EFix (f_name, p, e) ->
    (* if params length < 2 then x0 to x else xn to x.n *)
      (* "fix (" ^ f_name ^ ") =\n fun (" ^ Pat.to_burst_string p ^ ") ->\n"
      ^  *)
      to_burst_string ~d:(d + 1) e
  | _ -> failwith "unimplemented"
...
let get_param_ids e = 
  ...
  | EFix (f_name, p, e) -> 
     (* (match p with 
      | PVar id -> [id]
      | PTuple pats -> pats
      | _ -> failwith (Printf.sprintf "Not supported. params not tuple")) *)
      Pat.get_ids p
  | _ -> failwith (Printf.sprintf "Not supported non EFIX")

let rec replace exp (p, rep) =
  match exp with 
  | EVar id -> if (String.equal id p) then EVar rep else EVar id
  | ETuple es -> ETuple (List.map es ~f:(fun e -> replace e (p, rep)))
  | EOp (op, es) -> EOp (op, List.map es ~f:(fun e -> replace e (p, rep)))
  | EApp (f, arg) -> EApp (replace f (p, rep), replace arg (p, rep))
  | EMatch (scrut, branches) -> 
    EMatch (replace scrut (p, rep), List.map branches ~f:(fun (pat, e) -> (pat, replace e (p, rep))))
  | EFix (f_name, pt, e) -> 
    EFix ((if String.equal f_name "f0" then "f" else f_name), Pat.replace_pat pt (p, rep), replace e (p, rep))
  | _ -> failwith "unimplemented"

let rebuild e params =
  let param_map = 
    if (List.length params < 2) then
      match params with
      | [p] -> [(p, "x")]
      | _ -> failwith "Not supported. params not single"
      (* let p = List.hd params in
      [(p, "x")] *)
    else
      List.mapi params ~f:(fun i p -> (p, "x." ^ (string_of_int i)))
  in
  let param_map' = ("f0", "f") :: param_map in
  List.fold param_map' ~init:e ~f:(fun e' (p, rep) -> replace e' (p, rep))